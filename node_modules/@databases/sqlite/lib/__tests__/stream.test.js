...
test('streaming', async () => {
  await db.query(__1.sql`CREATE TABLE stream_values (id BIGINT NOT NULL PRIMARY KEY);`);
  const allValues = [];
  ...
  for await (const row of db.queryStream(__1.sql`SELECT * FROM stream_values`)) {
    results.push(row.id);
  }
